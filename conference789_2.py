...
class CommonTool:
    ...
    def on_cancel(self, mode):
        if self.serial_port and self.serial_port.is_open:
            ...
            if mode == 'show':
                messagebox.showinfo("關閉", "已關閉串口連線並且離開軟體控制模式")
        else:
            print("尚未連接串口，無需關閉")
    ...
    def save_config(self, mic_count_var, com_port_var):
        ...
        try:
            with open(CONFIG_FILE, "w", encoding="utf-8") as f:
                json.dump(config, f)
            messagebox.showinfo("儲存成功", f"已序列埠設定")
            # 進入軟體連線
    ...
    def save_camera_base_config(self, mic_count_var, com_port_var):
        ...
        try:
            with open(CONFIG_CAMERA_FILE, "w", encoding="utf-8") as f:
    ...
    def set_master_config(self, mic_id_var1, mic_id_var2):
        try:
            ...
            with open(CONFIG_MIC_FILE, "w", encoding="utf-8") as f:
    ...
    def set_time_config(self, hour_var, second_var):
        ...
        try:
            with open(CONFIG_TIME_FILE, "w", encoding="utf-8") as f:
                json.dump(config, f)
            messagebox.showinfo("儲存成功", f"已儲存時間設定")
        except Exception as e:
            messagebox.showerror("儲存錯誤", f"儲存發生錯誤： {e}")
    ...
    def save_queue_limit_config(self, mic_count_var1):
        ...
        try:
            with open(CONFIG_QUEUE_LIMIT_FILE, "w", encoding="utf-8") as f:
                json.dump(config, f, ensure_ascii=False, indent=4)
            messagebox.showinfo("儲存成功", "已儲存佇列表度限制設定")
        except Exception as e:
            messagebox.showerror("儲存錯誤", f"發生錯誤：{e}")
...
class ConferenceApp:
    ...
    def create_middle_buttons(self, parent):
        ...
        tk.Button(middle_frame, text="進入計時會議", width=15, height=5, fg="blue", command=self.go_to_conference_page).pack(side="left", padx=10)
        tk.Button(middle_frame, text="進入FIFO會議", width=15, height=5, fg="blue", command=self.go_to_fifoconference_page).pack(side="left", padx=10)
        tk.Button(middle_frame, text="離開", width=15, height=5, fg="blue",  command=self.close_window).pack(side="left", padx=10)
...
class SettingPage:
    ...
    def comportSettingPage(self):
        ...
        tk.Label(middle1_frame, text="麥克風數量：", font=("Arial", 12)).grid(row=0, column=0, sticky="e", pady=5,
                                                                               padx=10)
        self.mic_count_var = tk.IntVar(value=self.config.get("mic_count", 1))
        ...
        save_button = tk.Button(self.frame, text="儲存", width=12,
                                command=lambda: self.common.save_config(self.mic_count_var.get(), self.com_port_var.get()))
    ...
    def masterSettingPage(self):
        ...
        ## MIC ID 下拉選單
        if not os.path.exists(CONFIG_MIC_FILE):
            messagebox.showwarning("警告", "請先搜尋裝置列表")
            return
        # 讀取 mic_config.json
        ...
        # 預設值（若有現存兩個主席，分別預設）
        default_1 = self.common.hex_to_decimal(current_chairs[0]['mic_id']) if len(current_chairs) > 0 else \
        mic_ids_decimal[0]
        default_2 = self.common.hex_to_decimal(current_chairs[1]['mic_id']) if len(current_chairs) > 1 else \
        mic_ids_decimal[1]

        # 第一位主席
        tk.Label(middle1_frame, text="主席 MIC ID 1：", font=("Arial", 12)).grid(row=1, column=0, sticky="e", pady=5,
                                                                                padx=10)
        self.mic_id_var1 = tk.IntVar(value=default_1)
        ...
        tk.Label(middle1_frame, text="主席 MIC ID 2：", font=("Arial", 12)).grid(row=2, column=0, sticky="e", pady=5,
                                                                                padx=10)
        self.mic_id_var2 = tk.IntVar(value=default_2)
    ...
    def timeSettingPage(self):
        ...
        # 小時下拉選單在標籤前
        hour_var = tk.IntVar()
        hour_combobox = ttk.Combobox(middle1_frame, textvariable=hour_var, values=[str(i) for i in range(0, 13)], width=5,
                                     state="readonly")
        hour_combobox.pack(side="left", padx=5)
        hour_combobox.set(self.config.get("hour", 0))
    ...
    def build_generate_table(self, use_mic_list=False):
        try:
            ...
            self.config = self.common.load_config("mic") if os.path.exists(CONFIG_MIC_FILE) else []
...
class entryConferencePage:
            ...
            while self.common.is_serial_connected() and self.running:
                try:
                    ...
                    if incoming and len(incoming) == 4:
                        print(f'incoming: {incoming}   len: {len(incoming)}')
                        ...
                        if hex_msg != last_data:  # 避免重複列印
                            ...
                            if len(parts) >= 4 and parts[2] == '88' and parts[3] == '88':
                                ...
                                for item in sorted_queue:
                                    print(f'---------quere item: {item}')
                                    if item is not mic_id:
                                ...
                                #hex_command = "CC FF 2F 2F"
                                #command_bytes = bytes.fromhex(hex_command)
                                #self.common.serial_port.write(command_bytes)
                                # for micID in self.mic_id_queue:
                                #     print(f'micID: {micID}')
                                #     # 關閉other mic
                                #     self.send_mic_command(micID, 0)  # 關閉
                                #     self.timer_flags[micID] = False  # 停止倒數
                                #     time.sleep(0.1)
    ...
    def send_mic_command(self, mic_number, mode):
        ...
        filtered_ids = [item.get('mic_id') for item in self.mic_id_list if item.get("type") == 1]
        #label.config(text="麥克風開啟" if mode == 1 else "麥克風關閉", fg="blue" if mode == 1 else "red")
        if mic_number in filtered_ids and label:
            label.config(text="麥克風開啟" if mode == 1 else "麥克風關閉", fg="blue" if mode == 1 else "red")
        elif mic_number not in filtered_ids and label and mode == 0:
            label.config(text="麥克風關閉", fg="red")
    ...
    def controlCamera(self, mic_id):
        ...
        print(f'camera_id: {specify_camera_item.get("camera_id")}')
        if len(self.mic_id_queue) == 0:
            #print('ok')
            mic_id = 63
            for i in range(1, 7):
                #mic_id += 1
                ...
                command_bytes = bytes.fromhex(hex_command)
                self.common.camera_serial_port.write(command_bytes)
                print(f'33333 全部回原位 camera_cmd: {hex_command}')
                time.sleep(0.1)  # 稍微等一下
                # Log
                if hasattr(self.common, 'debug_window') and self.common.debug_window:
                    self.common.debug_window.add_log(f"攝影機{str(i)} 指令已送出： {hex_command}")
        else:
            #result = [item for item in self.mic_id_list if item['camera_id'] == specify_camera_item.get("camera_id")] # 找尋相對應的camera id的元素
            result = [
                item for item in self.mic_id_list
                if item.get('camera_id', 0) != 0 and item['camera_id'] == specify_camera_item.get("camera_id")
            ]

            mic_ids = [item['mic_id'] for item in result] # 只要取出id
            print(f'wwwww mic_ids: {mic_ids}')
            matched = [mid for mid in self.mic_id_queue if mid in mic_ids] # 找出有穩合相同群組的 camera_id ic
            print(f'qqqqq matched: {matched}')
            if matched:
                last_mic = matched[-1]
                print(f"last item: {last_mic}")
                print(f"last item type: {type(last_mic)}")
                # 這裡你可以加 camera 控制程式碼
                specify_camera_item = next((item for item in self.mic_id_list if "mic_id" in item and item["mic_id"] == last_mic), None)
                print(f'camera_id: { specify_camera_item.get("camera_id") }')
                hex_command = f'8{str(specify_camera_item.get("camera_id"))} 01 04 3F 02 {last_mic} FF'
                command_bytes = bytes.fromhex(hex_command)
                self.common.camera_serial_port.write(command_bytes)
                print(f'33333 攝影機{str(specify_camera_item.get("camera_id"))} 執行 camera_cmd: {hex_command}')
                # Log
                if hasattr(self.common, 'debug_window') and self.common.debug_window:
                    self.common.debug_window.add_log(f"攝影機{ specify_camera_item.get('camera_id') } 指令已送出： {hex_command}")
            else:
                print(f'{mic_id}')
                print(f'type: {type(mic_id)}')
                #convert_num = self.common.process_string(mic_id)
                convert_num = 63
                hex_number = self.common.decimal_to_hex(convert_num)
                hex_command = f'8{str(specify_camera_item.get("camera_id"))} 01 04 3F 02 {hex_number} FF'
                #hex_command = f'8{str(specify_camera_item.get("camera_id"))} 01 04 3F 02 61 FF'
                command_bytes = bytes.fromhex(hex_command)
                self.common.camera_serial_port.write(command_bytes)
                print(f'33333 攝影機{str(specify_camera_item.get("camera_id"))} 回原位 camera_cmd: {hex_command}')
                # Log
                if hasattr(self.common, 'debug_window') and self.common.debug_window:
                    self.common.debug_window.add_log(f"攝影機{ specify_camera_item.get('camera_id') } 指令已送出： {hex_command}")
...
class CameraMicMappingPage:
    ...
    def build_page(self):
        ...
        self.camera_count_var.set(self.camera_config.get("camera_count", 0))
        ...
        # 儲存按鈕
        save_button = tk.Button(self.frame, text="儲存", width=12, command=lambda: self.common.save_camera_mic_mapping(self.selected_camera_var, self.mic_vars))
...
class FIFOSettingPage:
    ...
    def build_set_page(self):
        ...
        self.mic_count_var1.set( self.mic_config.get("queue_limit_count", 0) )
        count_dropdown = ttk.Combobox(
...
class FIFOConferencePage:
                    ...
                    if incoming and len(incoming) == 4:
                        ...
                        if hex_msg != last_data:  # 避免重複列印
                            ...
                            if len(parts) >= 4 and parts[2] == '88' and parts[3] == '88':
                                ...
                                btn.config(text=f"主席: 麥克風 {mic_id_decimal} 已關閉", fg="green")
    ...
    def send_mic_command(self, mic_number, mode):
        ...
        ## 以下處理標簽
        print(f'mic_buttons: {self.mic_buttons}')
    ...
    def controlCamera(self, mic_id):
        ...
        if len(self.mic_id_queue1) == 0:
            ...
            mic_ids = [item['mic_id'] for item in result]  # 只要取出id
            print(f'wwwww mic_ids: {mic_ids}')
            matched = [mid for mid in self.mic_id_queue1 if mid in mic_ids]  # 找出有穩合相同群組的 camera_id ic
            print(f'qqqqq matched: {matched}')
            if matched:
                last_mic = matched[-1]
                print(f"last item: {last_mic}")
                print(f"last item type: {type(last_mic)}")
                # 這裡你可以加 camera 控制程式碼
                specify_camera_item = next(
                    (item for item in self.mic_id_list if "mic_id" in item and item["mic_id"] == last_mic), None)
                print(f'camera_id: {specify_camera_item.get("camera_id")}')
                hex_command = f'8{str(specify_camera_item.get("camera_id"))} 01 04 3F 02 {last_mic} FF'
                command_bytes = bytes.fromhex(hex_command)
                self.common.camera_serial_port.write(command_bytes)
